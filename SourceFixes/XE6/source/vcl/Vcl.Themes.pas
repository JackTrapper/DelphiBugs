...
type
  ...
  TSystemHook = (shMenus); // Enumeration elements will be expanded in the future
  ...
  public
    ...
    class property SystemHooks: TSystemHooks read FSystemHooks write SetSystemHooks default [shMenus];
  end;
...
begin
	{
		20140828	The ability to use Visual Styles is independant of whether we are using version 6 of the Common Controls library
...
begin
  Result := True;
  if not (shMenus in TStyleManager.SystemHooks) and (AClassName = PopupMenuClassName) then
    Result := False;
end;
...
begin
  ...
  begin
    ...
    if FRegSysStylesList.ContainsKey(sClassName) and TStyleManager.CheckSysClassName(sClassName) then
    begin
