...
{$WARN UNSUPPORTED_CONSTRUCT OFF} //ON}
{$WARN FILE_OPEN ON}
...
{$ELSE}
    ...
    begin
      ...
      begin
        DevMode := GlobalLock(DeviceMode);
        //20140908  Fix for QC107919: http://qc.embarcadero.com/wc/qcmain.aspx?d=107919
        //DevMode is uninitialized. This is fine for the "out" parameter.
        //But it is wrong (and an error) to pass an uninitialized DevMode to the second "in" parameter.
		  //The fix is to not pass a DevMode when where we didn't intent to pass one (i.e. pass nil).
        //And because the optional parameter is delcared as var, we have to go through the hoops to get the compiler to accept "nil"
//      if DocumentProperties(0, FPrinterHandle, ADevice, DevMode^, DevMode^, DM_OUT_BUFFER) < 0 then
		  if DocumentProperties(0, FPrinterHandle, ADevice, DevMode^, PDeviceModeW(nil)^, DM_OUT_BUFFER) < 0 then
        begin
          GlobalUnlock(DeviceMode);
          GlobalFree(DeviceMode);
          DeviceMode := 0;
